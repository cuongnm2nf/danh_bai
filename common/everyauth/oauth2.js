var everyModule = require('../../node_modules/everyauth/lib/modules/everymodule')
  , OAuth = require('../../node_modules/everyauth/node_modules/oauth').OAuth2
  , url = require('url')
  , URL = require('url')
  , querystring = require('querystring')
  , request = require('../../node_modules/everyauth/node_modules/request')
  , extractHostname = require('../../node_modules/everyauth/lib/utils').extractHostname;
...
everyModule.submodule('oauth2')
  ...
  .configurable({
    ...
    , basicAuth: 'use basic authen on url or not'
    , myModuleErrback: 'to overwrite connect callback'
  })
  ...
  .getAccessToken( function (code, data) {
    if (this._myModuleErrback) {
      this.moduleErrback(this._myModuleErrback);
    }
    var p = this.Promise()
      , params = {
            client_id: this._appId
    ...
    var opts = { url: url }
      ...
      if (this._basicAuth === true) {
          opts.url = URL.parse(opts.url);
          opts.url.auth = this._appId + ':' + this._appSecret;
      }
    switch (paramsVia) {
